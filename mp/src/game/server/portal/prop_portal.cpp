...
bool CProp_Portal::ShouldTeleportTouchingEntity(CBaseEntity *pOther)
{
	...
	// Adam HACKHACK: NPC collision is broken!
	/*
	if (m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(ptOtherCenter) < m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Dist) {
		if (!isGood) {
			DevMsg("===Dot Dist Compare is good!\n");
			isGood = true;
		}
		...
		if (isGood) {
			DevMsg("===Dot Dist compare is bad!\n");
			isGood = false;
		}
			
	} */
	//DevMsg("ptOtherCenter Dot is %d\n", static_cast<int>(m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(ptOtherCenter)));
	//DevMsg("mDist Dot is %d\n", static_cast<int>(m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Dist));
	if(m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot( ptOtherCenter ) < m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Dist) //|| ((m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Normal.Dot(ptOtherCenter) - 100.) < m_PortalSimulator.m_DataAccess.Placement.PortalPlane.m_Dist && !pOther->IsPlayer()))
	{
		...
		if( m_plane_Origin.normal.Dot( ptOtherCenter ) > m_plane_Origin.dist )
		{
			...
			if( SharedEnvironmentCheck( pOther ) )
			{
				...
				if (FClassnameIs(pOther, "npc_metropolice")) {
					...
					//if (!result.DidHit()) {
					if(pOther->owningPortal == NULL){
						DevMsg("Creating VPhys by Portal %i!\n", ((m_bIsPortal2) ? (2) : (1)));
						pOther->origAngles = pOther->GetAbsAngles();
						pOther->VPhysicsInitNormal(SOLID_BBOX, 0, false);
						pOther->ClearVPhysicsOnCollide = true;
