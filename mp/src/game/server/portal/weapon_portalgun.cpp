...
ConVar currentPortalId("currentPortalId", "1", FCVAR_HIDDEN | FCVAR_CHEAT | FCVAR_REPLICATED, "The Current Portal ID");
...
void CWeaponPortalgun::Activate()
{
	...
	if ( pPlayer )
	{
		...
		Assert( (m_iPortalLinkageGroupID >= 0) && (m_iPortalLinkageGroupID < 256) );
		m_iPortalLinkageGroupID = 1;
		currentPortalId.SetValue(m_iPortalLinkageGroupID);
	}
...
static void change_portalgun_linkage_id_f( const CCommand &args )
{
	...
	for( int i = 0; i != iWeaponCount; ++i )
	{
		...
		if( dynamic_cast<CWeaponPortalgun *>(pWeapon) != NULL )
		{
			...
			currentPortalId.SetValue(iNewID);
			DevMsg("Portal ID is now " + pPortalGun->m_iPortalLinkageGroupID);
			break;
		}
	}
}
