...
public:
	...
	void	ItemPostFrame();
	void	ItemBusyFrame();
	void AttachObjectAccessor(CBaseEntity *pObject, const Vector &pos);
...
void CWeaponPhysCannon::DetachObject( bool playSound, bool wasLaunched )
{
	...
	if ( playSound && !pOwner->m_bSilentDropAndPickup)
	{
...
void UpdateGrabControllerTargetPosition( CBasePlayer *pPlayer, Vector *vPosition, QAngle *qAngles )
{
	...
	if (!pGrabController)
		pGrabController = GetGrabControllerForPhysCannon(pPlayer->GetActiveWeapon());
...
void CWeaponPhysCannon::AttachObjectAccessor(CBaseEntity *pObject, const Vector &pos) {
	AttachObject(pObject, pos);
}
...
void PhysCannonPickupObject(CBasePlayer *pPlayer, CBaseEntity *pObject)
{
	...
	if (pCannon)
	{
		pCannon->AttachObjectAccessor(pObject, pObject->WorldSpaceCenter());
	}
}

void PhysCannonSilentAttachAndDetach(CBasePlayer *pPlayer, bool set)
{
	/*CWeaponPhysCannon *pCannon = dynamic_cast<CWeaponPhysCannon *>(pPlayer->GetActiveWeapon());
	if (pCannon)
	{
		pCannon->silentAttachAndDetach = set;
	} */

	CPortal_Player *pPortalPlayer = ToPortalPlayer(pPlayer);
	pPortalPlayer->m_bSilentDropAndPickup = set;
}