...
void CPhysicsCloneArea::EndTouch( CBaseEntity *pOther )
{
	...
	/* This code is now likely totally unneeded as the check for collision now falls entirely in CBaseEntity
	if (FClassnameIs(pOther, "npc_metropolice")) {
		if (pOther->owningPortal == m_pAttachedPortal) {
			DevMsg("Destroying VPhys!\n");
			//pOther->VPhysicsDestroyObject();
			//pOther->SetMoveType(MOVETYPE_STEP);
			pOther->ClearVPhysicsOnCollide = true;
			//pOther->owningPortal = NULL;
			// This has been moved to the clear area in order to allow easier checking of fall distance
		}
