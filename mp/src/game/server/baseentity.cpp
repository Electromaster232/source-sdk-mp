...
#ifdef PORTAL
ConVar ignorePortalFallDamage("sv_npcignoreportalfalldamage", "0", FCVAR_CHEAT);
#endif
...
#endif
#ifdef PORTAL
	// Reusing clear var as a sanity check
	if (ClearVPhysicsOnCollide && (!FClassnameIs(pHitEntity, "prop_portal") && FClassnameIs(this, "npc_metropolice"))) {
		DevMsg("Clearing VPhys\n");
		VPhysicsDestroyObject();
		SetMoveType(MOVETYPE_STEP);
		ClearVPhysicsOnCollide = false;
		Vector myPos = GetAbsOrigin();
		Vector theirPos = ((CBaseEntity *)owningPortal)->GetAbsOrigin();
		float dist = myPos.DistTo(theirPos);
		if (dist >= 50 && !ignorePortalFallDamage.GetBool())
			Event_Killed(CTakeDamageInfo());
		else {
			SetAbsAngles(origAngles);
