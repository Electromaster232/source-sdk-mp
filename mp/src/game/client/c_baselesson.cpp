...
bool CBaseLesson::IsLocked() const
{
	...
	return (gpGlobals->curtime > m_fStartTime + LESSON_MIN_TIME_BEFORE_LOCK_ALLOWED &&
		gpGlobals->curtime < fLockTime + m_fLockDuration);
}
...
void CIconLesson::Update()
{
	...
	if (!pLocatorTarget)
	{
		...
		Warning("Problem in lesson %s: Locator_GetTargetFromHandle returned null for handle %d.\n IsInstanceActive: "
			"%s. IsInstructing: %s. IsLearned: %s\n",
			GetName(), m_hLocatorTarget, (IsInstanceActive() ? "yes" : "no"), (IsInstructing() ? "yes" : "no"),
			(IsLearned() ? "yes" : "no"));
	...
	if (m_fOnScreenStartTime == 0.0f)
	{
		if (pLocatorTarget->IsOnScreen() &&
			(IsPresentComplete() || (pLocatorTarget->GetIconEffectsFlags() & LOCATOR_ICON_FX_STATIC)))
		{
...
void CIconLesson::UpdateLocatorTarget(CLocatorTarget *pLocatorTarget, C_BaseEntity *pIconTarget)
{
	...
	else
	{
		...
		pLocatorTarget->m_vecOrigin =
			pIconTarget->EyePosition() + MainViewUp() * m_flRelativeUpOffset + Vector(0.0f, 0.0f, m_flUpOffset);
	...
	if (!m_bFixedPosition &&
		((pLocalPlayer != NULL && pLocalPlayer == m_hIconTarget) || GetClientWorldEntity() == m_hIconTarget))
	{
...
#define LESSON_VARIABLE_INIT_SYMBOL(_varEnum, _varName, _varType)                                                      \
    g_n##_varEnum##Symbol = KeyValuesSystem()->GetSymbolForString(#_varEnum);

#define LESSON_SCRIPT_STRING_ADD_TO_MAP(_varEnum, _varName, _varType)                                                  \
    g_NameToTypeMap.Insert(#_varEnum, LESSON_VARIABLE_##_varEnum##);

// Create enum value
#define LESSON_VARIABLE_ENUM(_varEnum, _varName, _varType) LESSON_VARIABLE_##_varEnum##,

// Init info call
#define LESSON_VARIABLE_INIT_INFO_CALL(_varEnum, _varName, _varType)                                                   \
    g_pLessonVariableInfo[LESSON_VARIABLE_##_varEnum##].Init_##_varEnum##();
...
#define LESSON_SCRIPT_STRING_GENERAL(_varEnum, _varName, _varType)                                                     \
    LESSON_SCRIPT_STRING(LESSON_VARIABLE_##_varEnum##, #_varEnum)
...
#define PROCESS_LESSON_ACTION(_varEnum, _varName, _varType)                                                            \
    case LESSON_VARIABLE_##_varEnum##:                                                                                 \
        return ProcessElementAction(pLessonElement->iAction, pLessonElement->bNot, #_varName, _varName,                \
                                    &pLessonElement->szParam, eventParam_float);

#define PROCESS_LESSON_ACTION_EHANDLE(_varEnum, _varName, _varType)                                                    \
    case LESSON_VARIABLE_##_varEnum##:                                                                                 \
        return ProcessElementAction(pLessonElement->iAction, pLessonElement->bNot, #_varName, _varName,                \
                                    &pLessonElement->szParam, eventParam_float, eventParam_BaseEntity,                 \
                                    eventParam_string);

#define PROCESS_LESSON_ACTION_STRING(_varEnum, _varName, _varType)                                                     \
    case LESSON_VARIABLE_##_varEnum##:                                                                                 \
        return ProcessElementAction(pLessonElement->iAction, pLessonElement->bNot, #_varName, &_varName,               \
                                    &pLessonElement->szParam, eventParam_string);
...
#define LESSON_VARIABLE_GET_FROM_OFFSET(_type, _offset)                                                                \
    *static_cast<_type *>(static_cast<void *>(static_cast<byte *>(static_cast<void *>(this)) + _offset))
...
{
public:
	LessonVariableInfo() : iOffset(0), varType(FIELD_VOID)
	{
	}
...
#undef LESSON_VARIABLE_MACRO_STRING
		...
		else
		{
			...
			const CScriptedIconLesson *pLesson =
				static_cast<const CScriptedIconLesson *>(GetGameInstructor().GetLesson(GetName()));
...
void CScriptedIconLesson::ProcessOpenGameEvents(const CScriptedIconLesson *pRootLesson, const char *name,
	IGameEvent *event)
{
...
void CScriptedIconLesson::ProcessCloseGameEvents(const CScriptedIconLesson *pRootLesson, const char *name,
	IGameEvent *event)
{
...
void CScriptedIconLesson::ProcessSuccessGameEvents(const CScriptedIconLesson *pRootLesson, const char *name,
	IGameEvent *event)
{
...
void CScriptedIconLesson::InitElementsFromKeys(CUtlVector<LessonElement_t> *pLessonElements, KeyValues *pKey)
{
	...
	for (pSubKey = pKey->GetFirstSubKey(); pSubKey; pSubKey = pSubKey->GetNextKey())
	{
		...
		else
		{
			...
			pLessonElements->AddToTail(
				LessonElement_t(iVariable, iAction, bNot, bOptionalParam, pchParam, iParamVarIndex, paramType));
		}
	}
}

void CScriptedIconLesson::InitElementsFromElements(CUtlVector<LessonElement_t> *pLessonElements,
	const CUtlVector<LessonElement_t> *pLessonElements2)
{
...
bool CScriptedIconLesson::ProcessElement(IGameEvent *event, const LessonElement_t *pLessonElement, bool bInFailedScope)
{
	...
	{
		// Special case for setting the starting point for the lesson to stay locked from (we don't need variables for
		// this)
		if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
		{
	...
	case FIELD_FLOAT:
		if (pLessonElement->iParamVarIndex < LESSON_VARIABLE_TOTAL)
		{
			...
			case FIELD_BOOLEAN:
				...
				break;
			case FIELD_STRING:
				eventParam_float = static_cast<float>(
					atoi(&LESSON_VARIABLE_GET_FROM_OFFSET(CGameInstructorSymbol, pInfo->iOffset)->String()));
				break;
			case FIELD_EHANDLE:
			case FIELD_FUNCTION:
	...
	case FIELD_INTEGER:
		if (pLessonElement->iParamVarIndex < LESSON_VARIABLE_TOTAL)
		{
			...
			case FIELD_STRING:
				eventParam_float =
					atof(&LESSON_VARIABLE_GET_FROM_OFFSET(CGameInstructorSymbol, pInfo->iOffset)->String());
				break;
			case FIELD_EHANDLE:
			case FIELD_FUNCTION:
				DevWarning("Can't use this variable type with this parameter type in lesson script.\n");
	...
	case FIELD_STRING:
		if (pLessonElement->iParamVarIndex < LESSON_VARIABLE_TOTAL)
		{
			...
			{
			case FIELD_STRING:
				Q_strncpy(eventParam_string,
					&LESSON_VARIABLE_GET_FROM_OFFSET(CGameInstructorSymbol, pInfo->iOffset)->String(),
					sizeof(eventParam_string));
				break;
			case FIELD_FLOAT:
				Q_snprintf(eventParam_string, sizeof(eventParam_string), "%f",
					LESSON_VARIABLE_GET_FROM_OFFSET(float, pInfo->iOffset));
				break;
			case FIELD_INTEGER:
				Q_snprintf(eventParam_string, sizeof(eventParam_string), "%i",
					LESSON_VARIABLE_GET_FROM_OFFSET(int, pInfo->iOffset));
				break;
			case FIELD_BOOLEAN:
			case FIELD_EHANDLE:
			case FIELD_FUNCTION:
...
bool CScriptedIconLesson::ProcessElementAction(int iAction, bool bNot, const char *pchVarName, float &fVar,
...
bool CScriptedIconLesson::ProcessElementAction(int iAction, bool bNot, const char *pchVarName, int &iVar,
	const CGameInstructorSymbol *pchParamName, float fParam)
{
...
bool CScriptedIconLesson::ProcessElementAction(int iAction, bool bNot, const char *pchVarName, bool &bVar,
	const CGameInstructorSymbol *pchParamName, float fParam)
{
...
bool CScriptedIconLesson::ProcessElementAction(int iAction, bool bNot, const char *pchVarName, EHANDLE &hVar,
	const CGameInstructorSymbol *pchParamName, float fParam,
	C_BaseEntity *pParam, const char *pchParam)
{
	...
	case LESSON_ACTION_GET_DISTANCE: {
		if (!pVar || !pParam)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[output] = [%s]->DistTo( [%s] )", pchVarName,
					pchParamName->String());
	...
	case LESSON_ACTION_GET_ANGULAR_DISTANCE: {
		if (!pVar || !pParam)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[output] = [%s]->AngularDistTo( [%s] )", pchVarName,
					pchParamName->String());
				ConColorMsg(CBaseLesson::m_rgbaVerboseName, "...\n");
				ConColorMsg(CBaseLesson::m_rgbaVerboseClose, "\tVar handle or Param handle returned NULL!\n");
			}
	...
	case LESSON_ACTION_GET_PLAYER_DISPLAY_NAME: {
		...
		if (iTemp <= 0 || iTemp > 2)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\tQ_strcpy( [stringINVALID], [%s]->GetPlayerName() ",
					pchVarName);
				ConColorMsg(CBaseLesson::m_rgbaVerboseName, "... ");
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, ")\n");
		...
		if (!pVar)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\tQ_strcpy( [%s], [%s]->GetPlayerName() ",
		...
		*pString = pVar->GetPlayerName();
		...
		if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
		{
			ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\tQ_strcpy( [%s], [%s]->GetPlayerName() ", pchParamNameTemp,
				pchVarName);
	...
	case LESSON_ACTION_CLASSNAME_IS: {
		if (!pVar)
		{
			...
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain,
					(bNot) ? ("\t!FClassnameIs( [%s] ") : ("\tFClassnameIs( [%s] "), pchVarName);
				ConColorMsg(CBaseLesson::m_rgbaVerboseName, "...");
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, ", [%s] ", pchParamName->String());
	...
	case LESSON_ACTION_GET_ACTIVE_WEAPON: {
		...
		if (!pBaseCombatCharacter)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[%s] = [%s]->GetActiveWeapon()", pchParamNameTemp,
					pchVarName);
	...
	case LESSON_ACTION_WEAPON_IS: {
		...
		return (bNot) ? (Q_stricmp(pBaseCombatWeapon->GetName(), pchParam) != 0)
			: (Q_stricmp(pBaseCombatWeapon->GetName(), pchParam) == 0);
	}
	...
	case LESSON_ACTION_WEAPON_HAS: {
		...
		if (!pBaseCombatCharacter)
		{
			...
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain,
					(bNot) ? ("\t![%s]->Weapon_OwnsThisType([%s] ") : ("\t[%s]->Weapon_OwnsThisType([%s] "),
					pchVarName, pchParamName->String());
				ConColorMsg(CBaseLesson::m_rgbaVerboseName, "\"%s\"", pchParam);
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, ")\n");
				ConColorMsg(CBaseLesson::m_rgbaVerboseClose, "\tVar handle as BaseCombatCharacter returned NULL!\n");
		...
		return (bNot) ? (pBaseCombatCharacter->Weapon_OwnsThisType(pchParam) == NULL)
			: (pBaseCombatCharacter->Weapon_OwnsThisType(pchParam) != NULL);
	}
	...
	case LESSON_ACTION_GET_WEAPON_SLOT: {
		...
		if (!pBaseCombatCharacter)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[output] = [%s]->Weapon_GetSlot([%s] ", pchVarName,
					pchParamName->String());
				ConColorMsg(CBaseLesson::m_rgbaVerboseName, "\"%s\"", pchParam);
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, ") ...\n");
				ConColorMsg(CBaseLesson::m_rgbaVerboseClose, "\tVar handle as BaseCombatCharacter returned NULL!\n");
		...
		{
			ConColorMsg( CBaseLesson::m_rgbaVerbosePlain, "\t[output] = [%s]->Weapon_GetSlot([%s] ", pchVarName,
		pchParamName->String() ); ConColorMsg( CBaseLesson::m_rgbaVerboseName, "\"%s\"", pchParam ); ConColorMsg(
		CBaseLesson::m_rgbaVerbosePlain, ") " ); ConColorMsg( CBaseLesson::m_rgbaVerboseName, "%f\n", m_fOutput );
		}*/
	...
	case LESSON_ACTION_GET_WEAPON_IN_SLOT: {
		...
		if (!pBaseCombatCharacter)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[entity1] = [%s]->GetWeapon([%s] ", pchVarName,
					pchParamName->String());
	...
	case LESSON_ACTION_CLIP_PERCENTAGE_LESS_THAN: {
		...
		if (!pBaseCombatCharacter)
		{
			if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, "\t[%s]->GetActiveWeapon()->Clip1Percentage() ",
					pchVarName);
				ConColorMsg(CBaseLesson::m_rgbaVerboseName, "... ");
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain, (bNot) ? (">= [%s] ") : ("< [%s] "),
					pchParamName->String());
		...
		{
			fClip1Percentage = 100.0f * (static_cast<float>(pBaseCombatWeapon->Clip1()) /
				static_cast<float>(pBaseCombatWeapon->GetMaxClip1()));
		}
	...
	case LESSON_ACTION_WEAPON_AMMO_LOW: {
		...
		if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
		{
			...
			ConColorMsg(CBaseLesson::m_rgbaVerboseName, "%i ", iTemp);
			ConColorMsg(CBaseLesson::m_rgbaVerbosePlain,
				(bNot) ? (")->AmmoPercentage() >= 30 ") : (")->AmmoPercentage() < 30 "));
	...
	case LESSON_ACTION_WEAPON_AMMO_FULL: {
		...
		if (!pBasePlayer)
		{
			...
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain,
					(bNot) ? ("\t![%s]->GetWeaponInSlot( ") : ("\t[%s]->GetWeaponInSlot( "), pchVarName);
	...
	case LESSON_ACTION_WEAPON_AMMO_EMPTY: {
		...
		return (bNot) ? (!bAmmoEmpty) : (bAmmoEmpty);
										  ...
										  "\t[%s]->Weapon_CanUse([%s])\n" ), pchVarName, pchParamName->String() ); ConColorMsg(
										  CBaseLesson::m_rgbaVerboseClose, "\tVar handle as BasePlayer returned NULL!\n" );
												  }
											  ...
											  if ( !pBaseCombatWeapon )
											  {
												  if ( gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew() )
												  {
													  ConColorMsg( CBaseLesson::m_rgbaVerbosePlain, ( bNot ) ? ( "\t![%s]->Weapon_CanUse([%s])\n" ) : (
										  "\t[%s]->Weapon_CanUse([%s])\n" ), pchVarName, pchParamName->String() ); ConColorMsg(
										  CBaseLesson::m_rgbaVerboseClose, "\tParam BaseCombatWeapon returned NULL!\n" );
												  }
											  ...
											  {
												  ConColorMsg( CBaseLesson::m_rgbaVerbosePlain, ( bNot ) ? ( "\t![%s]->Weapon_CanUse([%s]) " ) : (
										  "\t[%s]->Weapon_CanUse([%s]) " ), pchVarName, pchParamName->String() ); ConColorMsg(
										  CBaseLesson::m_rgbaVerboseName, ( bCanEquip ) ? ( "true\n" ) : ( "false\n" ) );
											  }
	...
	case LESSON_ACTION_USE_TARGET_IS: {
		...
		if (!pBasePlayer)
		{
			...
			{
				ConColorMsg(CBaseLesson::m_rgbaVerbosePlain,
					(bNot) ? ("\tC_BaseEntity::Instance([%s]->GetUseEntity()) != [%s]\n")
		...
		return (bNot) ? (C_BaseEntity::Instance(pBasePlayer->GetUseEntity()) != pParam)
			: (C_BaseEntity::Instance(pBasePlayer->GetUseEntity()) == pParam);
	}
		...
		return true;
									   ...
									   /*case LESSON_ACTION_GET_POTENTIAL_USE_TARGET:
									   {
										   ...
										   if ( iTemp <= 0 || iTemp > 2 )
										   {
											   if ( gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew() )
											   {
												   ConColorMsg( CBaseLesson::m_rgbaVerbosePlain, "\t[entityINVALID] =
									   C_BaseEntity::Instance([%s]->GetPotentialUseEntity())\n", pchVarName ); ConColorMsg(
									   CBaseLesson::m_rgbaVerboseClose, "\tParam selecting string is out of range!\n" );
											   }
										   ...
										   if ( !pBasePlayer )
										   {
											   if ( gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew() )
											   {
												   ConColorMsg( CBaseLesson::m_rgbaVerbosePlain, "\t[%s] =
									   C_BaseEntity::Instance([%s]->GetPotentialUseEntity())\n", pchParamNameTemp, pchVarName ); ConColorMsg(
									   CBaseLesson::m_rgbaVerboseClose, "\tVar handle as Player returned NULL!\n" );
											   }
...
bool CScriptedIconLesson::ProcessElementAction(int iAction, bool bNot, const char *pchVarName,
	CGameInstructorSymbol *pchVar, const CGameInstructorSymbol *pchParamName,
	const char *pchParam)
{
	switch (iAction)
	{
	case LESSON_ACTION_REFERENCE_OPEN: {
		...
		if (gameinstructor_verbose.GetInt() > 0 && ShouldShowSpew())
		{
			ConColorMsg(Color(64, 128, 255, 255),
				(bNot) ? ("\t!( [\"%s\"]->IsInstanceActive() ") : ("\t( [\"%s\"]->IsInstanceActive() "),
				pchParamName->String());
...
#undef LESSON_VARIABLE_MACRO_STRING
	...
	CScriptedIconLesson::LessonActionMap.Insert("health percentage less than",
