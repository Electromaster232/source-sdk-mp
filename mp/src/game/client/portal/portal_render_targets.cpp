...
{
	//ConVarRef currentPortalId("currentPortalId");
	//switch (currentPortalId.GetInt()) {
	//case 1:
	//	return m_Portal1Texture;
	//	break;
	//case 2:
	//	return m_Portal3Texture;
	//	break;
	//}
	return m_Portal3Texture;
}

// Reuse portalgettexture functions, have check in get texture, create new init functions
// Or use iTexture swap contents??

// Use in game tools editor
// launch with -tools -nop4
...
ITexture* CPortalRenderTargets::InitPortal3Texture(IMaterialSystem* pMaterialSystem)
{
	if (IsX360())
	{
		// shouldn't be using
...
//	if ( materials->StencilBufferBits() == 0 )
	if ( IsPC() || !IsX360() )
	{
		...
		m_Portal3Texture.Init(InitPortal3Texture(pMaterialSystem));
	}
