...
void BuildIndividualMeshes( s_source_t *psource )
{
	...
	for (i = 0; i < psource->numvertices; i++)
	{
		...
		int k2;
		for( k2 = 0; k2 < MAXSTUDIOBONEWEIGHTS; k2++ )
		{
			psource->localBoneweight[i].bone[k2]		= g_bone[v_listdata[j].v].bone[k2];
			psource->localBoneweight[i].weight[k2]	= g_bone[v_listdata[j].v].weight[k2];
		}
