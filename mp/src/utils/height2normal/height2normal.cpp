				   ...
				   float bumpScale )
{
	...
	for( frameID = 0; frameID < numFrames; frameID++ )
	{
		...
		*pcUnderscore = NULL;
		...
		float sourceGamma;
		CUtlBuffer buf2;
		if ( !g_pFullFileSystem->ReadFile( heightTGAFileName, NULL, buf2 ) )
		{
			fprintf( stderr, "%s not found\n", heightTGAFileName );
			return;
		}

		if ( !TGALoader::GetInfo( buf2, &width, &height, &imageFormat, &sourceGamma ) )
		{
		...
		TGALoader::Load( pImageIA88, buf2, width, height, IMAGE_FORMAT_IA88, sourceGamma, false );
